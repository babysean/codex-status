...
메뉴 막대에는 현재 가장 중요한 한도가 `Codex 42%`처럼 표시됩니다. 메뉴를 열면 모든 사용량 한도, 초기화 시각, 크레딧 상태, 누적 토큰, 마지막 갱신 시각과 연결 상태를 확인할 수 있습니다.
...
앱을 실행하면 Dock 아이콘이나 일반 창 대신 메뉴 막대에 `Codex …%`가 나타납니다.

- 메뉴 막대의 `Codex …%`를 클릭하면 상세 사용량을 볼 수 있습니다.
- **지금 새로고침**을 누르면 즉시 최신 값을 조회합니다.
- 앱을 닫으려면 메뉴 아래쪽의 **종료**를 누릅니다.
- 데이터가 아직 없거나 연결할 수 없으면 `Codex —`로 표시됩니다.
...
실행 후 메뉴 막대에서 `Codex …%` 항목을 찾습니다. 터미널에서 실행한 경우 `Control-C`로 종료할 수 있습니다.
...
### 메뉴 막대에 `Codex —`가 계속 표시됩니다

1. ChatGPT/Codex 앱이나 Codex CLI에서 로그인되어 있는지 확인합니다.
2. Codex Status 메뉴의 **지금 새로고침**을 누릅니다.
3. Codex Status를 종료했다가 다시 실행합니다.
...
터미널에서 `codex login`을 실행하거나 ChatGPT/Codex 앱에서 다시 로그인한 뒤 새로고침하세요.
